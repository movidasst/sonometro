(() => {
  ...
  async function submitMemberLogin(event) {
    ...
    try {
      ...
      submit.querySelector("span").textContent = "Ingresar al simulador";
    }
  }
  ...
  function renderMenu() {
    ...
    list.innerHTML = items.map((item, index) => {
      ...
      return `<div class="menu-item ${index === state.menuIndex ? "selected" : ""}" role="option" aria-selected="${index === state.menuIndex}"><span>${label}</span><small>${value}</small></div>`;
    }).join("");
  }
  ...
  function renderSpectrum() {
    ...
    $("spectrumBars").innerHTML = levels.map((level, index) => {
      ...
      return `<div class="bar-wrap ${index === selected ? "selected" : ""}" title="${frequencies[index]} Hz: ${level.toFixed(1)} dB"><i class="spectrum-bar" style="height:${height}%"></i><span class="bar-label">${frequencies[index]}</span></div>`;
    }).join("");
